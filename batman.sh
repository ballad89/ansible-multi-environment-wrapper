...
function load_inventory {
    ...
    export ANSIBLE_AG_ENVIRONMENT="$1"
    ...
    if [[ ! "$PS1" == *"\$ANSIBLE_AG_ENVIRONMENT"* ]]
    then
       export PS1="$PS1 (ANSIBLE: \$ANSIBLE_AG_ENVIRONMENT) >> "
    fi
    # create the password file if it doesn't exist
    # this will prompt
    if ! [ -f "$ANSIBLE_VAULT_PASSWORD_FILE" ]
    then
       read -s -p "VAULT PASSWORD FOR '$ANSIBLE_AG_ENVIRONMENT' [ENTER]>" VAULT_PW
       echo ""
